...
MAJOR_VERSION = 0
MINOR_VERSION = 9.2 
LAST_UPDATED  = datetime.date(year=2022, month=4, day=28)
...
# Changes in v0.9.2
#  * became gangsta
#  * added boundedScale to image class because scaleImage sucks for bounds
...
class App(object):
    ...
    def boundedScale(app, image, x1, y1, x2, y2, antialias=False):
        resample = Image.ANTIALIAS if antialias else Image.NEAREST
        imageBounds = (round(abs(x2-x1)),round(abs(y2-y1)))
        return image.resize(imageBounds, resample=resample)
